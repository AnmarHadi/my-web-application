...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/api/contractors").then(res => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/vehicles?${query.join("&")}`);
  ...
  const columnsRtl = React.useMemo(() => {
